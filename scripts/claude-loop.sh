...
dry_run_show() {
  ...
  if [[ -n "${TASK_QUEUE_FILE}" ]]; then
    ...
    echo "  claude command: claude --append-system-prompt \"claude-loop started this session. Your first action must be to invoke the /continue skill using the Skill tool.\""
  fi
}
...
while [[ "${LOOP_RUNNING}" == "true" ]]; do
  ...
  CLAUDE_CMD=("claude" "--append-system-prompt" "claude-loop started this session. Your first action must be to invoke the /continue skill using the Skill tool.")
