...
- SessionStart automatically injects memory and context at session start including Current Work and Next Steps from the previous session. Use `/continue` to pick up where the last session left off — it reads the injected context and starts working immediately.
