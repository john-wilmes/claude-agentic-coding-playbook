...
unitTest("U1. admins -> deny with users guidance", () => {
  const r = checkPhantomCollection("mcp__mongodb__find", { collection: "admins" });
  assert.ok(r, "Expected deny result");
  assert.strictEqual(r.action, "deny");
  assert.ok(r.reason.includes("users"), `Missing 'users': ${r.reason}`);
  assert.ok(r.reason.includes('type: "admin"'), `Missing type filter: ${r.reason}`);
});

unitTest("U2. unknown collection -> null (no phantom match)", () => {
  const r = checkPhantomCollection("mcp__mongodb__find", { collection: "widgets" });
...
unitTest("U4. orders collection -> null (allow)", () => {
  const r = checkPhantomCollection("mcp__mongodb__find", { collection: "orders" });
  assert.strictEqual(r, null);
});

unitTest("U5. aggregate on admins -> deny", () => {
  const r = checkPhantomCollection("mcp__mongodb__aggregate", { collection: "admins", pipeline: [] });
...
unitTest("U14. source hex -> null (string exception)", () => {
  const r = checkBareObjectId("mcp__mongodb__find", {
    filter: { source: SAMPLE_HEX },
  });
  assert.strictEqual(r, null, `Expected null (source is string field), got: ${JSON.stringify(r)}`);
});
...
unitTest("U50. @rootId filter -> rewrite to @tenantId", () => {
  const r = checkDatadogPhantomFilters("mcp__datadog__get_logs", {
    filters: { "@rootId": "abc123", service: "my-service" },
  });
  assert.ok(r);
  assert.strictEqual(r.action, "update");
  assert.deepStrictEqual(r.updatedInput.filters, { "@tenantId": "abc123", service: "my-service" });
  assert.ok(r.warning.includes("@rootId"));
});

unitTest("U51. @orgId filter -> rewrite to @tenantId", () => {
  const r = checkDatadogPhantomFilters("mcp__datadog__get_logs", {
    filters: { "@orgId": "def456" },
  });
  assert.ok(r);
  assert.strictEqual(r.action, "update");
  assert.deepStrictEqual(r.updatedInput.filters, { "@tenantId": "def456" });
  assert.ok(r.warning.includes("@orgId"));
});

unitTest("U52. @tenantId filter -> null (no rewrite needed)", () => {
  const r = checkDatadogPhantomFilters("mcp__datadog__get_logs", {
    filters: { "@tenantId": "abc123" },
  });
  assert.strictEqual(r, null);
});
...
unitTest("U55. both @rootId and @orgId -> both rewritten, last wins for @tenantId value", () => {
  const r = checkDatadogPhantomFilters("mcp__datadog__get_logs", {
    filters: { "@rootId": "aaa", "@orgId": "bbb" },
  });
  assert.ok(r);
  assert.strictEqual(r.action, "update");
  // Both map to @tenantId; last one overwrites
  assert.ok(r.updatedInput.filters["@tenantId"]);
  assert.ok(r.warning.includes("@rootId"));
  assert.ok(r.warning.includes("@orgId"));
});
...
unitTest("U41. phantom collection + missing limit -> deny (guard before autofix)", () => {
  const r = checkMcpDataCall("mcp__mongodb__find", { collection: "admins", filter: { type: "admin" } });
  assert.ok(r && r.action === "deny", `Expected deny, got: ${JSON.stringify(r)}`);
});
...
test("I1. admins collection -> deny with users guidance", (env) => {
  const result = runGuard("mcp__mongodb__find", { collection: "admins", filter: { type: "admin" } }, env);
  assertBlocked(result, "admins collection");
  const reason = getDenyReason(result);
  assert.ok(reason.includes("users"), `Missing 'users': ${reason}`);
});

test("I2. admins collection -> deny reason mentions type filter", (env) => {
  const result = runGuard("mcp__mongodb__find", { collection: "admins", filter: {} }, env);
  assertBlocked(result, "admins collection with type");
  const reason = getDenyReason(result);
  assert.ok(reason.includes("type"), `Missing 'type': ${reason}`);
  assert.ok(reason.includes("admin"), `Missing 'admin': ${reason}`);
});
...
test("I4. orders collection -> allow", (env) => {
  seedDiscovery(env, ["orders"]);
  const result = runGuard("mcp__mongodb__find", {
    collection: "orders",
    filter: { orgId: "abc123" },
    limit: 10,
  }, env);
  assertAllowed(result, "orders collection");
});

test("I5. aggregate on admins -> deny", (env) => {
  const result = runGuard("mcp__mongodb__aggregate", {
    collection: "admins",
    pipeline: [{ $match: {} }, { $limit: 5 }],
  }, env);
  assertBlocked(result, "aggregate on admins");
});
...
// ─── Integration tests: phantom collection miscasing ─────────────────────────

console.log("\nmcp-data-guard.js (phantom collection — case sensitivity):");

test("admins collection (exact match) -> deny with redirect", (env) => {
  const r = runGuard("mcp__mongodb__find", { collection: "admins", filter: { type: "admin" } }, env);
  assertBlocked(r, "admins exact");
  assert.ok(getDenyReason(r).includes("users"), "should mention real collection");
});

test("unknown collection not in phantom map -> deny with discover_collection hint", (env) => {
  // Phantom guard passes; discovery guard fires since collection not discovered
  const r = runGuard("mcp__mongodb__find", { collection: "widgets", filter: { tenantId: "123" }, limit: 10 }, env);
  assertBlocked(r, "unknown collection");
  assert.ok(getDenyReason(r).includes("discover_collection"), "should mention discover_collection");
});
...
unitTest("U46. checkPhantomCollection detects admins phantom", () => {
  const r = checkPhantomCollection("mcp__mongodb__find", { collection: "admins" });
  assert.ok(r !== null);
  assert.ok(r.reason.includes("users"), `Missing 'users' in reason: ${r.reason}`);
});

unitTest("U47. checkAppointmentStatus is a no-op by default (override to add schema-specific guards)", () => {
  // checkAppointmentStatus is intentionally a no-op in the generic hook.
  // Customize it with your own schema-specific status validation.
  const r = checkAppointmentStatus("mcp__mongodb__find", { collection: "orders", filter: { status: "invalid" } });
  assert.strictEqual(r, null);
});

unitTest("U48. checkAppointmentStatus no-op on any collection", () => {
  const r = checkAppointmentStatus("mcp__mongodb__find", { collection: "orders", filter: { status: "confirmed" } });
  assert.strictEqual(r, null);
});

unitTest("U49. checkAppointmentStatus no-op regardless of tool", () => {
  const r = checkAppointmentStatus("mcp__mongodb__aggregate", { collection: "orders", pipeline: [] });
  assert.strictEqual(r, null);
});
