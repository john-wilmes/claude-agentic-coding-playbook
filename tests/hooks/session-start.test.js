...
// Test 23: Auto-start directive injected when Current Work has next steps
test("23. auto-start directive injected when memory has next steps", () => {
  const { home, cleanup } = createTempHome();
  try {
    const dir = createProjectDir({ git: true });
    try {
      ...
      // Write MEMORY.md with next steps (numbered list)
      const cwdEncoded = dir.replace(/:/g, "-").replace(/[\\/]/g, "-").replace(/^-/, "");
      ...
      const result = runHook(
        HOOK_MODULE,
        { session_id: "test-autostart-1", cwd: dir },
        { HOME: home, USERPROFILE: home }
      );

      assert.strictEqual(result.status, 0);
      const ctx = result.json.hookSpecificOutput.additionalContext;
      assert.ok(ctx.includes("AUTO-START DIRECTIVE"), "Should inject auto-start directive");
      assert.ok(ctx.includes("Fix the login bug"), "Should include the next steps content");
      // Directive should appear BEFORE the current work content
      const directiveIdx = ctx.indexOf("AUTO-START DIRECTIVE");
      const contentIdx = ctx.indexOf("Current Work");
      assert.ok(directiveIdx < contentIdx, "Directive should precede Current Work content");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  } finally {
...
// Test 24: No auto-start directive when Current Work has no next steps
test("24. no auto-start directive when memory lacks next steps", () => {
  const { home, cleanup } = createTempHome();
  try {
    const dir = createProjectDir({ git: true });
    try {
      ...
      // Write MEMORY.md without next steps
      const cwdEncoded = dir.replace(/:/g, "-").replace(/[\\/]/g, "-").replace(/^-/, "");
      const memDir = path.join(home, ".claude", "projects", cwdEncoded, "memory");
      fs.mkdirSync(memDir, { recursive: true });
      fs.writeFileSync(path.join(memDir, "MEMORY.md"), [
      ...
      const result = runHook(
        HOOK_MODULE,
        { session_id: "test-autostart-2", cwd: dir },
        { HOME: home, USERPROFILE: home }
      );

      assert.strictEqual(result.status, 0);
      const ctx = result.json.hookSpecificOutput.additionalContext;
      assert.ok(!ctx.includes("AUTO-START DIRECTIVE"), "Should NOT inject auto-start directive");
      assert.ok(ctx.includes("All tasks done"), "Should still include current work content");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  } finally {
